...
public class RealSizeCommand extends AbstractPaperCommand {
    ...
    public void register() {
        var builder = this.commandManager.commandBuilder("realsize")
                .commandDescription(Description.of(paradisu.messagesConfig().commands().realSize().helpMsg()));
        ...
        this.commandManager.command(builder.literal("reset")
                .handler(context -> {
    ...
    public void realSize(CommandContext<CommandSourceStack> context) {
        ...
        if (totalinches <= 76 && totalinches >= 60) { // this is in inches
            ...
            player.sendMessage(Messages.prefixed(
                    MiniMessage.miniMessage().deserialize(paradisu.messagesConfig().commands().realSize().output().get(0))
            ));


        } else if (totalinches >= 76) {
            player.sendMessage(Messages.prefixed(
                    MiniMessage.miniMessage().deserialize(paradisu.messagesConfig().commands().realSize().tootall())
            ));
        } else {
            player.sendMessage(Messages.prefixed(
                    MiniMessage.miniMessage().deserialize(paradisu.messagesConfig().commands().realSize().tooshort())
            ));
        }
    }
